...
export default {
    ...
    methods: {
        testConnection() {
            ...
            fetch('./api.php', {
                ...
                body: JSON.stringify({ action: 'test-connection', host: this.host, port: this.port, name: this.name, user: this.user, pass: this.pass })
            })
                .then(res => res.json())
                .then(res => {
    ...
    render(h) {
        ...
        const field = (label, id, model, type = 'text', ref = null, note = null) =>
            h('div', { class: 'w-full mt-3' }, [
                ...
                note ? h('p', { class: 'text-xs italic mt-1' }, note) : null,
            ])

        return h('div', { class: 'min-h-screen flex' }, [
            sidebar,
            h('div', { class: 'flex-1 bg-gray-100 py-8 px-16' }, [
                h('h1', { class: 'text-3xl font-medium' }, ['Install New ', h('slims-text'), h('span', { class: 'text-gray-500 text-lg' }, [h('span', { class: 'pl-4 pr-2' }, '\u2014'), ' 1 of 2'])]),
                ...
                h('form', { class: 'w-full max-w-xl', on: { submit: e => { e.preventDefault(); this.testConnection() } } }, [
                    ...
                    field('Database name', 'db_name', 'name', 'text', 'db_name', 'Notes: If this database name not exist, SLiMS will try to create it for you. Make sure you have this privilege.'),
